        ...
        while True:
            ...
            if user_input == "1":
                try:
                    ...
                    if n_input > 0:
                        sorted_aircraft = Aircraft.get_sort(aircraft_list)[:n_input]
                        print(f"\nТоп {n_input} самолётов по высоте:")
                        for i, aircraft in enumerate(sorted_aircraft, 1):
                            print(
                                f"{i}. Бортовой номер: {aircraft.callsign or 'Нет данных'}, "
                ...
                if not param:
                    ...
                    continue
                result = [aircraft for aircraft in aircraft_list if param.lower() in (aircraft.country or "").lower()]
                if result:
                    ...
                    for i, aircraft in enumerate(result, 1):
                        ...
                        print(f"{i}. Бортовой номер: {callsign}, " f"Высота: {height} м, " f"Скорость: {velocity} м/с")
                else:
                ...
                if valid_velocity:
                    ...
                    print(
                        f"Самый быстрый самолет: {fastest.callsign or 'Нет данных'} "
                        f"со скоростью {fastest.velocity:.1f} м/с"
                    )
                    print(
                        f"Самый медленный самолет: {slowest.callsign or 'Нет данных'} "
                ...
                if valid_height:
                    ...
                    print(
                        f"Самый высокий самолет: {highest.callsign or 'Нет данных'} "
                        f"на высоте {highest.height:.1f} м"
                    )
                    print(
                        f"Самый низкий самолет: {lowest.callsign or 'Нет данных'} " f"на высоте {lowest.height:.1f} м"
                    )
                else:
